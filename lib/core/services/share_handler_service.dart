  ...
  static Future<void> _processYoutubePlaylist(String url) async {
    ...
    try {
      ...
      try {
        ...
        print('[ShareHandler] Initial tracks: ${parsedTracks.length}, continuation: ${continuation != null}');
      } catch (e) {
        print('[ShareHandler] Error finding initial continuation: $e');
      }
      while (continuation != null) {
        pageCount++;
        print('[ShareHandler] Fetching continuation page $pageCount...');
        final pageData = await _fetchYoutubeContinuationPage(continuation);
        if (pageData == null) {
          print('[ShareHandler] Continuation page $pageCount returned null');
          break;
        }
        final before = parsedTracks.length;
        _findTracksInJson(pageData, parsedTracks);
        print('[ShareHandler] Page $pageCount: added ${parsedTracks.length - before} tracks (total: ${parsedTracks.length})');
        try {
          continuation = _findContinuationToken(pageData);
        } catch (e) {
          print('[ShareHandler] Error finding continuation: $e');
          break;
        }
      }
      print('[ShareHandler] Total tracks fetched: ${parsedTracks.length} across ${pageCount + 1} pages');
  ...
  static Future<Map<String, dynamic>?> _fetchYoutubeContinuationPage(String token) async {
    ...
    try {
      ...
      if (response.statusCode != 200 || response.data == null) {
        print('[ShareHandler] Continuation API returned ${response.statusCode}');
        return null;
      }
      return response.data as Map<String, dynamic>;
    } catch (e) {
      print('[ShareHandler] Continuation API error: $e');
      return null;
    }
  }
